...
// Connessione DB
$mysqli = new mysqli("localhost", "root", "", "pesca_inventory");
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    $query = $mysqli->prepare("SELECT photo FROM inventory WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $result = $mysqli->query("SELECT id, item, type, size, photo FROM inventory ORDER BY id DESC");
    $prodotti = [];
...
if ($photo_path) {
    $stmt = $mysqli->prepare("INSERT INTO inventory (item, type, size, photo) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $item, $type, $size, $photo_path);
} else {
    $stmt = $mysqli->prepare("INSERT INTO inventory (item, type, size) VALUES (?, ?, ?)");
